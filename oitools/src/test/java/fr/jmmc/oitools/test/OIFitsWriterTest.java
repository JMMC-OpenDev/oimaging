...
public class OIFitsWriterTest implements TestEnv {
    ...
    private final static boolean COMPARE_FITS = true;
...
//      final String ext = ".oifits";
            ...
            n++;
            final OIFitsFile srcOIFitsFile = loadFile(fileSrc + ext);
            if (srcOIFitsFile == null) {
                ...
                // verify and check :
                final OIFitsFile destOIFitsFile = loadFile(fileTo);
            ...
            if (directory.exists() && directory.isDirectory()) {
                ...
                for (File f : files) {
                    if (f.isFile() && (f.getName().endsWith("fits") || f.getName().endsWith("fits.gz"))) {
                        ...
                        final OIFitsFile srcOIFitsFile = loadFile(fileSrc);
                        if (srcOIFitsFile == null) {
                            errors++;
                        } else {
                            ...
                            // compare fits files at fits level (header / data) :
                            if (COMPARE_FITS && !TamFitsTest.compareFile(fileSrc, fileTo)) {
                                errors++;
                            }
                            ...
                            if (true) {
                                // Fix file source:
                                srcOIFitsFile.setAbsoluteFilePath(fileSrc);
                                final OIFitsFile destOIFitsFile = loadFile(fileTo);
                                if (destOIFitsFile == null) {
                                    errors++;
                                } else if (!OITableUtils.compareOIFitsFile(srcOIFitsFile, destOIFitsFile)) {
