...
public final class AsciiTable extends Data implements TableData {
  ...
  public void fillHeader(Header hdr) {
    ...
    try {
      ...
      hdr.setBitpix(8, "8-bit bytes");
      hdr.setNaxes(2, "2-dimensional ascii table");
      hdr.setNaxis(1, rowLen, "width of row in bytes");
      hdr.setNaxis(2, nRows, "number of rows in table");
      Cursor iter = hdr.iterator();
      iter.setKey("NAXIS2");
      iter.next();
      iter.add("PCOUNT", new HeaderCard("PCOUNT", 0, "no group data"));
      iter.add("GCOUNT", new HeaderCard("GCOUNT", 1, "one data group (required keyword)"));
      iter.add("TFIELDS", new HeaderCard("TFIELDS", nFields, "number of fields in each row"));
