...
public final class Header implements FitsElement {
   ...
   * @since 2005-10-24
   */
  public void updateLines(final Header newHdr) throws fr.nom.tam.fits.HeaderCardException {
  ...
  public String getStringValue(String key) {
    ...
    while (append) {
      ...
      if (nxt == null) {
        ...
        if (key == null || comm == null || !key.equals("COMMENT")) {
          append = false;
        } else {
...
// BUG FIX : LAURENT :
// Skip length test (need == 80) to skip extra bytes ...
//	        if (firstCard && need == 80) {
          if (firstCard) {
            if (need != 80) {
              System.err.println("Warning: file has " + (80 - need) + " extra byte after last HDU");
            }
// BUG FIX : LAURENT : END
            throw e;
          }
        ...
        if (firstCard) {
          ...
          if (key == null || (!key.equals("SIMPLE") && !key.equals("XTENSION"))) {
            throw new IOException("Header[" + key + "] : Not FITS format at " + fileOffset + ":" + cbuf);
          }
          firstCard = false;
        }
...
//	        throw new IOException("Invalid FITS Header", e);
  ...
  private int getAdjustedLength(String in, int max) {
    ...
    int size = 0;
    int i;
    for (i = 0; i < in.length() && size < max; i++) {
      if (in.charAt(i) == '\'') {
        size += 2;
        if (size > max) {
  ...
  protected void addLongString(String key, String val, String comment)
    ...
    while (val != null && val.length() > 0) {
      ...
      iter.add(new HeaderCard("COMMENT", null, curr));
    }
  }
  ...
  void nullImage() {
    iter = iterator();
    try {
      addValue("SIMPLE", true, "Java FITS : " + new Date());
      addValue("BITPIX", 8, "number of bits per data pixel");
      addValue("NAXIS", 0, "number of data axes");
      addValue("EXTEND", true, "FITS dataset may contain extensions");
    } catch (HeaderCardException e) {
    }
  }
   ...
   */
  public void setSimple(final boolean val) {
    deleteKey("SIMPLE");
    deleteKey("XTENSION");
    ...
    if (findCard("NAXIS") != null) {
      ...
      if (findCard("NAXIS" + nax) != null) {
        ...
        try {
          removeCard("EXTEND");
          iter.add("EXTEND", new HeaderCard("EXTEND", true, "FITS dataset may contain extensions"));
    ...
    iter = iterator();
    try {
      iter.add("SIMPLE", new HeaderCard("SIMPLE", val, "Java FITS : " + new Date()));
    } catch (HeaderCardException e) {
   ...
   */
  public void setXtension(final String val) {
    deleteKey("SIMPLE");
    deleteKey("XTENSION");
    deleteKey("EXTEND");
    iter = iterator();
    try {
      if ("BINTABLE".equals(val)) {
        iter.add("XTENSION", new HeaderCard("XTENSION", val, "binary table extension"));
      } else {
        iter.add("XTENSION", new HeaderCard("XTENSION", val, "Java FITS: " + new Date()));
      }
    } catch (HeaderCardException e) {
   ...
   * </ul>
   */
  public void setBitpix(final int val) {
    setBitpix(val, null);
  }
   ...
   * <li> -64 -- IEEE 64 bit floating point numbers.
   * </ul>
   * @param comment keyword's comment
   */
  public void setBitpix(final int val, final String comment) {
    iter = iterator();
    iter.next();
    try {
      iter.add("BITPIX", new HeaderCard("BITPIX", val, comment));
    } catch (HeaderCardException e) {
   ...
   */
  public void setNaxes(final int val) {
    setNaxes(val, null);
  }
   ...
   * @param comment keyword's comment
   */
  public void setNaxes(final int val, final String comment) {
    iter.setKey("BITPIX");
    if (iter.hasNext()) {
      iter.next();
    }
    ...
    try {
      iter.add("NAXIS", new HeaderCard("NAXIS", val, comment));
    } catch (HeaderCardException e) {
   ...
   */
  public void setNaxis(final int axis, final int dim) {
    setNaxis(axis, dim, null);
  }
   ...
   * @param comment keyword's comment
   */
  public void setNaxis(final int axis, final int dim, final String comment) {
    ...
    try {
      iter.add("NAXIS" + axis, new HeaderCard("NAXIS" + axis, dim, comment));
