...
public class ColumnMeta extends CellMeta {
    ...
    public ColumnMeta(final String name, final String desc, final Types dataType, final int repeat,
        ...
        this(name, desc, dataType, repeat, false, false, NO_INT_VALUES, NO_STR_VALUES, unit, errName, null);
    }
    ...
    private ColumnMeta(final String name, final String desc, final Types dataType,
        ...
        super(MetaType.COLUMN, name, desc, dataType, optional, is3D, repeat, intAcceptedValues, stringAcceptedValues, unit);
    ...
    public final boolean isArray() {
        return getRepeat() > 1 || this instanceof ArrayColumnMeta;
    }
    ...
    public final void check(final Object value, final int nbRows, final OIFitsChecker checker) {
        ...
        // Check values:
        checkValues(value, columnRows, columnRepeat, checker);
    }
     ...
     * @param checker checker component
     */
    private void checkValues(final Object value, final int columnRows, final int columnRepeat, final OIFitsChecker checker) {
        boolean error;
        ...
        if (intAcceptedValues.length != 0) {
            ...
            for (int rowNb = 0; rowNb < columnRows; rowNb++) {
                ...
                for (int r = 0, rlen = values.length; r < rlen; r++) {
                    if (!ModelBase.isUndefined(values[r])) {
                        error = true;
            ...
            if (getDataType() == Types.TYPE_REAL) {
                ...
                checker.severe("Incompatible data type " + getDataType() + " with positive values for column '" + this.getName() + "' ...");
            }
        }
    }
}
