...
public final class FitsImageLoader {
    ...
    public static List<FitsImageHDU> processHDUnits(final String filename, final Collection<BasicHDU> hdus,
        ...
        for (BasicHDU hdu : hdus) {
            ...
            if (hdu instanceof ImageHDU) {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Skipped " + hdu.getClass().getSimpleName() + '#' + i + " [" + filename + "]");
                }
            }
    ...
    public static FitsImageHDU processHDUnit(final OIFitsChecker checker, final String filename,
        ...
        if (nAxis > 3) {
            logger.info("Skipped image in ImageHDU#" + hduIndex + " [" + filename
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("FITS CUBE ImageHDU#" + hduIndex + " [" + filename + "] - NAXIS3 = " + nAxis3);
            }
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("FITS IMAGE ImageHDU#" + hduIndex + " [" + filename + ']');
            }
    ...
    private static FitsImageHDU createImageHDU(final OIFitsChecker checker, final String filename,
        ...
        for (int imageIndex = 1; imageIndex <= imgCount; imageIndex++) {
            ...
            if (image.getNbRows() <= 0 || image.getNbCols() <= 0) {
                logger.info("Skipped image in ImageHDU#" + hduIndex + " [" + filename + "][" + imageIndex + "/" + imgCount
            ...
            if (!hasKeyword || OIFitsChecker.isInspectRules()) {
                if (!keyword.isOptional()) {
                    ...
                    if (checker != null) {
                        ...
                        checker.severe("Missing keyword '" + name + "'");
                    }
                }
            }
            if (hasKeyword || OIFitsChecker.isInspectRules()) {
                // parse keyword value:
                value = parseKeyword(checker, keyword, header.getValue(name), hduFits);
     ...
     */
    private static Object parseKeyword(final OIFitsChecker checker, final KeywordMeta keyword, final String keywordValue, final FitsHDU hduFits) {
        if (logger.isLoggable(Level.FINE)) {
        ...
        if (dataType == Types.TYPE_CHAR) {
            ...
            return (keywordValue != null) ? Boolean.valueOf(keywordValue.startsWith("T")) : null;
        }
        // Numeric keywords:
        Number value = null;
        ...
        if (kDataType != keyword.getDataType() || OIFitsChecker.isInspectRules()) {
            if (checker != null) {
                ...
                checker.ruleFailed(Rule.GENERIC_KEYWORD_FORMAT, hduFits, keyword.getName());
                checker.severe("Invalid format for keyword '" + keyword.getName() + "', found '" + kDataType.getRepresentation() + "' should be '" + keyword.getType() + "'");
            }
        }
        if (value == null) {
            if (keywordValue == null) {
    ...
    private static void processImageKeywords(final Header header, final FitsImage image, final int imageIndex, final boolean requireCdeltKeywords) throws FitsException, IllegalArgumentException {
        ...
        if (image.getNbCols() > 0 && image.getNbRows() > 0 && requireCdeltKeywords
            ...
            logger.warning(" Missing keyword(s) [ "
                    ...
                    + " ] in HDU #" + image.getFitsImageHDU().getExtNb());
