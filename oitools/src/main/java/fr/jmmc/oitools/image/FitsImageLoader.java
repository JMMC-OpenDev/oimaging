...
public final class FitsImageLoader {
     ...
     * @throws IllegalArgumentException if unsupported unit or unit conversion is not allowed or missing CDELT keyword
     */
    private static void processHDUnits(final FitsImageFile imgFitsFile, final List<BasicHDU> hdus, final boolean firstOnly, final boolean requireCdeltKeywords) throws FitsException, IOException, IllegalArgumentException {
     ...
     * @throws IllegalArgumentException if unsupported unit or unit conversion is not allowed or missing CDELT keyword
     */
    public static List<FitsImageHDU> processHDUnits(final String filename, final Collection<BasicHDU> hdus, final boolean firstOnly, final boolean requireCdeltKeywords) throws FitsException, IOException, IllegalArgumentException {
        ...
        for (BasicHDU hdu : hdus) {
            ...
            if (hdu instanceof ImageHDU) {
                ...
                if (nAxis > 3) {
                    logger.info("Skipped ImageHDU#" + i + " [" + filename
                    ...
                    if (logger.isLoggable(Level.FINE)) {
                        logger.fine("FITS CUBE ImageHDU#" + i + " [" + filename + "] - NAXIS3 = " + nAxis3);
                    }
                    ...
                    for (int imageIndex = 1; imageIndex <= nAxis3; imageIndex++) {
                        ...
                        image.setFitsImageIdentifier(filename + '#' + i + '-' + imageIndex + '/' + nAxis3);
                        image.setImageIndex(imageIndex);

                        // load image:
                        processImage(imgHdu, image, imageIndex, requireCdeltKeywords);

                        // skip empty images:
                        if (image.getNbRows() <= 0 || image.getNbCols() <= 0) {
                            logger.info("Skipped ImageHDU#" + i + " [" + filename + "][" + imageIndex + "/" + nAxis3
                    ...
                    if (logger.isLoggable(Level.FINE)) {
                        logger.fine("FITS IMAGE ImageHDU#" + i + " [" + filename + ']');
                    }
                    ...
                    // define the fits image identifier:
                    image.setFitsImageIdentifier(filename + '#' + i);

                    // load image:
                    processImage(imgHdu, image, UNDEFINED_INDEX, requireCdeltKeywords);
                ...
                logger.info("Skipped " + hdu.getClass().getSimpleName() + '#' + i + " [" + filename + "]");
            }
     ...
     * @throws IllegalArgumentException if unsupported unit or unit conversion is not allowed or missing CDELT keyword
     */
    private static void processImage(final ImageHDU hdu, final FitsImage image, final int imageIndex, final boolean requireCdeltKeywords) throws FitsException, IllegalArgumentException {
        if (logger.isLoggable(Level.FINE)) {
            logger.fine("processImage: " + image);
        }

        // get Fits header :
        processKeywords(hdu.getHeader(), image, imageIndex, requireCdeltKeywords);
     ...
     * @throws IllegalArgumentException if unsupported unit or unit conversion is not allowed or missing CDELT keyword
     */
    private static void processKeywords(final Header header, final FitsImage image, final int imageIndex, final boolean requireCdeltKeywords) throws FitsException, IllegalArgumentException {
        ...
        if (image.getNbCols() > 0 && image.getNbRows() > 0 && requireCdeltKeywords && (header.getValue(FitsImageConstants.KEYWORD_CDELT1) == null || header.getValue(FitsImageConstants.KEYWORD_CDELT2) == null)) {
            logger.warning(" Missing keyword(s) [ "
                    + FitsImageConstants.KEYWORD_CDELT1 + "=" + header.getStringValue(FitsImageConstants.KEYWORD_CDELT1) + " or " + FitsImageConstants.KEYWORD_CDELT2 + "=" + header.getStringValue(FitsImageConstants.KEYWORD_CDELT2)
                    + " ] in HDU #" + image.getFitsImageHDU().getHduIndex());
            throw new IllegalArgumentException(" Missing keyword(s) [ " + FitsImageConstants.KEYWORD_CDELT1 + " or " + FitsImageConstants.KEYWORD_CDELT2 + " ] in HDU #" + image.getFitsImageHDU().getHduIndex());
        }
        ...
        // Handle optional HDUNAME if present
        final String hduname = header.getTrimmedStringValue(ImageOiConstants.KEYWORD_HDUNAME);
        if (hduname != null) {
            image.getFitsImageHDU().setHduName(hduname);
        }

        // Copy all header cards:
        copyKeywords(header, image.getFitsImageHDU());
    }

    /**
     * Process the image header to copy all keywords
     * @param header image header
     * @param imageHDU Fits image HDU container
     * @throws FitsException if any FITS error occurred
     */
    public static void copyKeywords(final Header header, final FitsImageHDU imageHDU) throws FitsException {
        if (imageHDU != null) {
