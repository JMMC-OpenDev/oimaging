...
public final class FitsImageWriter {
    ...
    static {
        ...
        SKIP_KEYWORDS.remove(FitsConstants.KEYWORD_HISTORY);
        SKIP_KEYWORDS.remove(FitsConstants.KEYWORD_COMMENT);
    }
     ...
     * @throws IOException IO failure
     */
    public static void createHDUnits(final Fits fitsFile, final List<FitsImageHDU> imageHDUs,
    ...
    private static void processKeywords(final Header header, final FitsImage image) throws FitsException {
        ...
        if (imageHDU != null) {
            final String hduName = imageHDU.getHduName();
            if (hduName != null) {
                header.addValue(ImageOiConstants.KEYWORD_HDUNAME, hduName, ImageOiConstants.KEYWORD_DESCRIPTION_HDUNAME);
            }

            final List<FitsHeaderCard> headerCards = imageHDU.getHeaderCards();
