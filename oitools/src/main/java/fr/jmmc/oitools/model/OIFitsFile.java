    ...
    public void check(final OIFitsChecker checker) {
        ...
        if (isOIFits2()) {
            if (getPrimaryImageHDU() == null || OIFitsChecker.isInspectRules()) {
                // rule [PRIMARYHDU_EXIST_V2] check if the main header (PRIMARY HDU) exists in the OIFITS 2 file
                checker.ruleFailed(Rule.PRIMARYHDU_EXIST_V2, OIFitsConstants.PRIMARY_HDU, OIFitsStandard.VERSION_2);
        ...
        if (!hasOiTarget() || OIFitsChecker.isInspectRules()) {
            // rule [OI_TARGET_EXIST] check if only one OI_TARGET table exists in the OIFITS file
            checker.ruleFailed(Rule.OI_TARGET_EXIST, OIFitsConstants.TABLE_OI_TARGET, (FitsConstants.KEYWORD_CONTENT_OIFITS2.equals(FitsConstants.KEYWORD_CONTENT) ? OIFitsStandard.VERSION_2 : OIFitsStandard.VERSION_1));
        ...
        if (this.insNameToOiWavelength.isEmpty() || OIFitsChecker.isInspectRules()) {
            // rule [OI_WAVELENGTH_EXIST] check if at least one OI_WAVELENGTH table exists in the OIFITS file
            checker.ruleFailed(Rule.OI_WAVELENGTH_EXIST, OIFitsConstants.TABLE_OI_WAVELENGTH, (FitsConstants.KEYWORD_CONTENT_OIFITS2.equals(FitsConstants.KEYWORD_CONTENT) ? OIFitsStandard.VERSION_2 : OIFitsStandard.VERSION_1));
        ...
        if (isOIFits2()) {
            ...
            if (this.arrNameToOiArray.isEmpty() || OIFitsChecker.isInspectRules()) {
                // rule [OI_ARRAY_EXIST_V2] check if at least one OI_ARRAY table exists in the OIFITS 2 file
                checker.ruleFailed(Rule.OI_ARRAY_EXIST_V2, OIFitsConstants.TABLE_OI_ARRAY, OIFitsStandard.VERSION_2);
            ...
            if (OIFitsChecker.isInspectRules()) {
                ...
                checker.ruleFailed(Rule.OI_INSPOL_INSNAME_UNIQ, OIFitsConstants.TABLE_OI_INSPOL, OIFitsStandard.VERSION_2);
            }
        }
    ...
    public void checkCrossReference(final OITable oiTable, final OIFitsChecker checker) {
        if (oiTable instanceof OITarget) {
            ...
            if (oitarget.getNbTargets() < 1 || OIFitsChecker.isInspectRules()) {
                // rule [TARGET_EXIST] check if the OI_TARGET table have at least one target
                checker.ruleFailed(Rule.TARGET_EXIST, oitarget, OIFitsConstants.COLUMN_TARGET);
            ...
            if (insName != null) {
                ...
                final List<OIWavelength> oiwaves = this.insNameToOiWavelength.get(insName);

                if (isInspectRules()) {
                    checker.ruleFailed(Rule.INSNAME_REF, oiwavelength, OIFitsConstants.KEYWORD_INSNAME);
                    checker.ruleFailed(Rule.INSNAME_UNIQ, oiwavelength, OIFitsConstants.KEYWORD_INSNAME);
                }
                ...
                if (oiwaves == null) {
                    ...
                    for (Iterator<OIWavelength> it = oiwaves.iterator(); it.hasNext();) {
                        sb.append('|').append(it.next().getExtNb());
                    }
                    // rule [INSNAME_UNIQ] check if a single OI_WAVELENGTH table corresponds to the INSNAME keyword
                    checker.severe("OI_WAVELENGTH tables [" + sb.toString().substring(1) + "] are identified by same INSNAME='" + oiwavelength.getInsName() + "'");
                }
            } else {
                // already checked
            }
            ...
            if (isInspectRules()) {
                checker.ruleFailed(Rule.GENERIC_ARRNAME_REF, oiarray, OIFitsConstants.KEYWORD_ARRNAME);
                checker.ruleFailed(Rule.ARRNAME_UNIQ, oiarray, OIFitsConstants.KEYWORD_ARRNAME);
            }

            if (arrName != null) {
                /* Get OiArray associated to ARRNAME value */
                final List<OIArray> arrayList = arrNameToOiArray.get(arrName);
                ...
                if (arrayList == null) {
                    ...
                    checker.severe("Missing OI_ARRAY table that describes the '" + arrName + "' array");
                } else if (arrayList.size() > 1) {
                    /* Problem: more that one OiArray table associated to ARRNAME value, that is strictly forbiden */
                    final StringBuilder sb = new StringBuilder();

                    for (Iterator<OIArray> it = arrayList.iterator(); it.hasNext();) {
                        sb.append('|').append(it.next().getExtNb());
                    }
                    // rule [ARRNAME_UNIQ] check if a single OI_ARRAY table corresponds to the ARRNAME keyword

                    checker.severe("OI_ARRAY tables [" + sb.toString().substring(1) + "] are identified by same ARRNAME='" + oiarray.getArrName() + "'");
                }
            } else {
            ...
            if (isInspectRules()) {
                checker.ruleFailed(Rule.CORRNAME_REF, oicorr, OIFitsConstants.KEYWORD_CORRNAME);
                checker.ruleFailed(Rule.CORRNAME_UNIQ, oicorr, OIFitsConstants.KEYWORD_CORRNAME);
            }

            if (corrName != null) {
                /* Get OICorr associated to CORRNAME value */
                final List<OICorr> listCorrname = corrNameToOiCorr.get(corrName);

                if (listCorrname == null) {
                ...
                } else if (listCorrname.size() > 1) {
                    /* Problem: more that one OICorr table associated
                         * to CORRNAME value, that is strictly forbiden */
                    final StringBuilder sb = new StringBuilder();

                    for (Iterator<OICorr> it = listCorrname.iterator(); it.hasNext();) {
                        sb.append('|').append(it.next().getExtNb());
                    }
                    // rule [CORRNAME_UNIQ] check if a single OI_CORR table corresponds to the CORRNAME keyword
                    checker.severe("OI_CORR tables [" + sb.toString().substring(1) + "] are identified by same CORRNAME='" + oicorr.getCorrName() + "'");
                }
            } else {
                // already checked
            }
