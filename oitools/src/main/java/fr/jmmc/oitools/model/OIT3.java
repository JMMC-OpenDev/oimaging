    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        for (int i = 0, j; i < nRows; i++) {
            ...
            for (j = 0; j < nWaves; j++) {
                if (!rowFlag[j]) {
                    // Not flagged:
                    if (!isErrorValid(checker, rowT3ampErr[j], this, OIFitsConstants.COLUMN_T3AMPERR)) {
                    ...
                    if (!isErrorValid(checker, rowT3PhiErr[j], this, OIFitsConstants.COLUMN_T3PHIERR)) {
        ...
        if (corrindx_T3amp != null || corrindx_T3phi != null) {
            ...
            if (oiCorr == null || OIFitsChecker.isInspectRules()) {
                ...
                checker.severe("Missing OI_CORR table but the column CORRINDX_T3AMP or CORRINDX_T3PHI is defined.");
            }
            if (oiCorr != null || OIFitsChecker.isInspectRules()) {
                // column is defined
