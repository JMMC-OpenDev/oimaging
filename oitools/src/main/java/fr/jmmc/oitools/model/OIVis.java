...
public final class OIVis extends OIData {
    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        boolean[] rowFlag;
        double[] rowVisampErr, rowVisPhiErr, rowRVisErr, rowIVisErr;

        for (int i = 0, j; i < nRows; i++) {
            rowFlag = flags[i];
            rowVisampErr = visAmpErr[i];
            rowVisPhiErr = visPhiErr[i];
            rowRVisErr = (rvisErr != null) ? rvisErr[i] : null;
            rowIVisErr = (ivisErr != null) ? ivisErr[i] : null;

            for (j = 0; j < nWaves; j++) {
                if (!rowFlag[j]) {
                    // Not flagged:
                    if (!isErrorValid(checker, rowVisampErr[j], this, OIFitsConstants.COLUMN_VISAMPERR)) {
                    ...
                    if (!isErrorValid(checker, rowVisPhiErr[j], this, OIFitsConstants.COLUMN_VISPHIERR)) {
                    ...
                    if (rowRVisErr != null && !isErrorValid(checker, rowRVisErr[j], this, OIFitsConstants.COLUMN_RVISERR)) {
                        checker.severe("Invalid value at index " + j + " for column '" + OIFitsConstants.COLUMN_RVISERR
                                + "' line " + i + ", found '" + rowRVisErr[j] + "' should be >= 0 or NaN or flagged out");
                    }
                    if (rowIVisErr != null && !isErrorValid(checker, rowIVisErr[j], this, OIFitsConstants.COLUMN_IVISERR)) {
                        checker.severe("Invalid value at index " + j + " for column '" + OIFitsConstants.COLUMN_IVISERR
                                + "' line " + i + ", found '" + rowIVisErr[j] + "' should be >= 0 or NaN or flagged out");
                    }
                }
            }
        }
