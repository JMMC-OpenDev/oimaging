    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        for (int i = 0; i < nRows; i++) {
            ...
            if (idxI < 1 || OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_CORR_IINDEX_MIN, this, OIFitsConstants.COLUMN_IINDX);
            ...
            if (idxJ <= idxI || OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_CORR_JINDEX_SUP, this, OIFitsConstants.COLUMN_JINDX);
            ...
            if (idxI > ndata || OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_CORR_IJINDEX_MAX, this, OIFitsConstants.COLUMN_IINDX);
                checker.severe("IINDX index [" + idxI + "] cannot be > NDATA [" + ndata + "] at row " + i);
            }
            if (idxJ > ndata || OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_CORR_IJINDEX_MAX, this, OIFitsConstants.COLUMN_JINDX);
