...
public final class OIPrimaryHDU extends FitsImageHDU {
    ...
    private void checkValueContains(final OIFitsChecker checker, final String keywordName, final String value) {
        if (OIFitsChecker.isInspectRules()) {
            checker.ruleFailed(Rule.PRIMARYHDU_MULTI_TARGET, this, keywordName);
        }
        if (getKeyword(keywordName) != null) {
            if (!getKeyword(keywordName).startsWith(value)) {
                checker.severe(keywordName + " keyword must contain the value '" + value + "' when multi targets and array");
            }
        }
    }
    ...
    void checkBookkeepingKeyword(final OIFitsChecker checker) {
        if (OIFitsChecker.isInspectRules()) {
            ...
            if (getKeyword(FitsConstants.KEYWORD_REFERENC) == null) {
                checker.warning(FitsConstants.KEYWORD_REFERENC + " keyword must be present");
            }
            if (getKeyword(OIFitsConstants.KEYWORD_PROG_ID) == null) {
                checker.warning(OIFitsConstants.KEYWORD_PROG_ID + " keyword must be present");
            }
            if (getKeyword(OIFitsConstants.KEYWORD_PROCSOFT) == null) {
                checker.warning(OIFitsConstants.KEYWORD_PROCSOFT + " keyword must be present");
            }
            if (getKeyword(OIFitsConstants.KEYWORD_OBSTECH) == null) {
                checker.warning(OIFitsConstants.KEYWORD_OBSTECH + " keyword must be present");
            }
        }
    }
