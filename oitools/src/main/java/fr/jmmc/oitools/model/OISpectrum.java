...
public final class OISpectrum extends OIData {
    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        // TODO: use FLUX dimensions (Nwave = 0 if missing table)
        final int nWaves = getNWave();

        final boolean[][] flags = getFlag();
        final double[][] fluxErr = getFluxErr();
        ...
        for (int i = 0, j; i < nRows; i++) {
            rowFlag = flags[i];
            rowFluxErr = fluxErr[i];

            for (j = 0; j < nWaves; j++) {
                if (!rowFlag[j]) {
                    // Not flagged:
                    if (!isErrorValid(checker, rowFluxErr[j], this, OIFitsConstants.COLUMN_FLUXERR)) {
                        checker.severe("Invalid value at index " + j + " for column '" + OIFitsConstants.COLUMN_FLUXERR
                                + "' line " + i + ", found '" + rowFluxErr[j] + "' should be >= 0 or NaN or flagged out");
                    }
                }
            }
        }
