    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        if (OIFitsConstants.KEYWORD_FRAME_GEOCENTRIC.equalsIgnoreCase(getFrame()) || OIFitsChecker.isInspectRules()) {
            ...
            if ((Double.isNaN(norm) || norm <= MIN_EARTH_RADIUS) || OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_ARRAY_XYZ, this, OIFitsConstants.KEYWORD_ARRAY_X);
                checker.ruleFailed(Rule.OI_ARRAY_XYZ, this, OIFitsConstants.KEYWORD_ARRAY_Y);
                checker.ruleFailed(Rule.OI_ARRAY_XYZ, this, OIFitsConstants.KEYWORD_ARRAY_Z);
                checker.warning("Invalid ARRAY_X/Y/Z keywords: " + Arrays.toString(arrayXYZ));

                checker.ruleFailed(Rule.OI_ARRAY_XYZ_FIX, this, OIFitsConstants.KEYWORD_ARRAY_X);
                checker.ruleFailed(Rule.OI_ARRAY_XYZ_FIX, this, OIFitsConstants.KEYWORD_ARRAY_Y);
                checker.ruleFailed(Rule.OI_ARRAY_XYZ_FIX, this, OIFitsConstants.KEYWORD_ARRAY_Z);
        ...
        for (int i = 0; i < staIndexes.length; i++) {
            if (staNames[i] == null || OIFitsChecker.isInspectRules()) {
                ...
                checker.ruleFailed(Rule.OI_ARRAY_STA_NAME, this, OIFitsConstants.COLUMN_STA_NAME);
                checker.severe("STA_NAME undefined at row " + i);
            }
            if (staNames[i] != null || OIFitsChecker.isInspectRules()) {
                ...
                if (refId < 1 || OIFitsChecker.isInspectRules()) {
                    checker.ruleFailed(Rule.OI_ARRAY_STA_INDEX_MIN, this, OIFitsConstants.COLUMN_STA_INDEX);
                    checker.warning("STA_INDEX[" + refId + "] cannot be < 1 at row " + i);
                }
                for (int j = i + 1; j < staIndexes.length; j++) {
                    ...
                    if (refId == staIndexes[j] || OIFitsChecker.isInspectRules()) {
                        checker.ruleFailed(Rule.OI_ARRAY_STA_INDEX_UNIQ, this, OIFitsConstants.COLUMN_STA_INDEX);
                        checker.severe("STA_INDEX[" + refId + "] duplicated at rows " + i + " | " + j);
                    }
                    ...
                    if (refName.equals(staNames[j]) || OIFitsChecker.isInspectRules()) {
                        checker.ruleFailed(Rule.OI_ARRAY_STA_NAME_UNIQ, this, OIFitsConstants.COLUMN_STA_NAME);
                        checker.severe("STA_NAME[" + refName + "] duplicated at rows " + i + " | " + j);
                    }
                }
            }
        }
