...
public class OIFitsLoader {
    ...
    private void processHDUnits(final OIFitsStandard std, final BasicHDU[] hdus, String absFilePath) throws FitsException, IOException, IllegalArgumentException {
                    ...
                    if (oiTable != null) {
                        ...
                        if (!oiFitsFile.isOIFits2()) {
                            ...
                            if (OIFitsChecker.isInspectRules()) {
                                checker.ruleFailed(Rule.TABLE_NOT_OIFITS2, OIFitsConstants.TABLE_OI_CORR, OIFitsStandard.VERSION_1);
                                checker.ruleFailed(Rule.TABLE_NOT_OIFITS2, OIFitsConstants.TABLE_OI_INSPOL, OIFitsStandard.VERSION_1);
                            }
            ...
            if (hdu instanceof BinaryTableHDU) {
                ...
                if (oiTable != null) {
                    ...
                    if (!oiFitsFile.isOIFits2()) {
                        // rule [TABLE_NOT_OIFITS2] check if any OIFITS 2 specific table (OI_CORR, OI_INSPOL or OI_FLUX) is present in the OIFITS 1 file
                        if (OIFitsChecker.isInspectRules()) {
                            checker.ruleFailed(Rule.TABLE_NOT_OIFITS2, OIFitsConstants.TABLE_OI_FLUX, OIFitsStandard.VERSION_1);
                        }
        ...
        for (int i = 0; i < nbHDU; i++) {
            if (!processed[i] || OIFitsChecker.isInspectRules()) {
                ...
                if (!processed[i] || OIFitsChecker.isInspectRules()) {
                    // rule [TABLE_UNKNOWN] check if the table belongs to the OIFITS standard and version
                    checker.ruleFailed(Rule.TABLE_UNKNOWN, "FILE", FitsConstants.KEYWORD_CONTENT_OIFITS2.equals(hdu.getHeader().getTrimmedStringValue(FitsConstants.KEYWORD_CONTENT)) ? OIFitsStandard.VERSION_2 : OIFitsStandard.VERSION_1);
    ...
    private void processData(final BinaryTableHDU hdu, final OITable table) throws FitsException {
        ...
        for (int i = 0, nCols = hdu.getNCols(); i < nCols; i++) {
            ...
            if (!columnsDesc.containsKey(name) || OIFitsChecker.isInspectRules()) {
    ...
    private Object parseColumn(final ColumnMeta column, final char columnType,
            ...
            final Object columnValue, final OITable table) {
        ...
        int descRepeat = column.getRepeat();

        // rule [GENERIC_COL_FORMAT] check if the column format matches the expected format (data type & dimensions)
        if (OIFitsChecker.isInspectRules()) {
            checker.ruleFailed(Rule.GENERIC_COL_FORMAT, table, column.getName());
            checker.ruleFailed(Rule.GENERIC_COL_DIM, table, column.getName());
            checker.ruleFailed(Rule.GENERIC_COL_UNIT_EXIST, table, column.getName());
            checker.ruleFailed(Rule.GENERIC_COL_UNIT, table, column.getName());
        }
        if (descRepeat == 0) {
        ...
        if (column.getUnits() != parsedUnit) {
            if (parsedUnit == Units.NO_UNIT) {
                // rule [GENERIC_COL_UNIT_EXIST] check if the column unit exist
                if (column.getUnit().isEmpty()) {
                    checker.warning("Missing unit for column '" + column.getName() + "', should be not empty");
                } else {
                ...
                if (column.getUnits() instanceof CustomUnits) {
                    ...
                    // store the user unit
                    if (parsedUnit != null) {
                        columnUnits.setRepresentation(parsedUnit.getStandardRepresentation());
                    } else {
                        columnUnits.setRepresentation(columnUnit);
                    }
                } else {
