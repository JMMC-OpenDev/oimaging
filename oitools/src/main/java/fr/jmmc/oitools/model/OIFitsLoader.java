...
//            this.checker.addFileIssue(CheckRule.FILE_NOT_LOADED, absFilePath, fe.getMessage());
     ...
     */
    private void processHDUnits(final BasicHDU[] hdus) throws FitsException, IOException, IllegalArgumentException {
        ...
        if (nbHDU != 0) {
            if (hdus[0] instanceof ImageHDU) {
                ...
                this.oiFitsFile.setPrimaryImageHdu(imageHDU);
        ...
        for (int i = 1; i < nbHDU; i++) {
            ...
            if (hdu instanceof BinaryTableHDU) {
                ...
                if (oiTable != null) {
                    ...
                    logger.log(Level.INFO, "Analysing table [{0}]:", OITable.getTableId(extName, i));
        ...
        for (int i = 1; i < nbHDU; i++) {
            ...
            if (hdu instanceof BinaryTableHDU) {
                ...
                if (oiTable != null) {
                    ...
                    // TODO remove hardcoded section for V2.0
                    if (oiTable instanceof OISpectrum) {
                        this.checker.severe("Unsupported table in OIFITS V1.0");
        ...
        ImageOiData imageOiData = new ImageOiData();;
        ...
        for (int i = 0; i < nbHDU; i++) {
            ...
            if (!processed[i]) {
                ...
                // TODO add some checking code
                if (extName != null) {
                    if (extName.equals(ImageOiConstants.EXTNAME_IMAGE_OI_INPUT_PARAM)) {
                        this.processKeywords(hdu.getHeader(), imageOiData.getInputParam());
                    } else if (extName.equals(ImageOiConstants.EXTNAME_IMAGE_OI_OUTPUT_PARAM)) {
                        this.processKeywords(hdu.getHeader(), imageOiData.getOutputParam());
                    }
                } else {
                    this.checker.warning("Skipping non-standard OIFITS table [" + i + "]: " + extName);

                    // TODO fill non standard HDU list
                    if (logger.isLoggable(Level.INFO)) {
        ...
        // Load fits images with mandatory CDELT as we respect IMAGE-OI interface
        List<FitsImageHDU> fitsImageHDUs = FitsImageLoader.processHDUnits(this.oiFitsFile.getName(), hduList, false, true);
        if (fitsImageHDUs.size() > 0) {
            // Store reference into oifitsFile
            this.oiFitsFile.setImageOiData(imageOiData);

            // reaffect primaryHDU with whole informations
            if (fitsImageHDUs.get(0).getHduIndex() == 0) {
                this.oiFitsFile.setPrimaryImageHdu(fitsImageHDUs.get(0));
            }

            // and register tables
            imageOiData.registerTables(fitsImageHDUs);
        }
     ...
     * @throws FitsException if any FITS error occurred
     */
    private void processKeywords(final Header header, final Table table) throws FitsException {
    ...
    private Object parseKeyword(final KeywordMeta keyword, final String keywordValue) {
        ...
        if (dataType == Types.TYPE_CHAR) {
            ...
            return Boolean.valueOf(keywordValue.startsWith("T"));
        } else {
            Number value = null;
