...
public class JELEval extends ExpressionEvaluator {
    ...
    public double[][] eval(final OIData oiData, final String colNameEval,
        ...
        if (_logger.isLoggable(Level.FINE)) {
            _logger.fine("eval: column= " + colNameEval + " expression= " + expression);
        }
        ...
        try {
            ...
            if (_logger.isLoggable(Level.FINE)) {
                _logger.fine("usedNames: " + usedNames);
            }
            ...
            for (ColumnMeta colMeta : columnsDescCollection) {
                ...
                if (!colName.equalsIgnoreCase(colNameEval)) {
                    ...
                    if (usedNames.contains(colName)) {
                        ...
                        if (_logger.isLoggable(Level.FINE)) {
                            _logger.fine("colMeta: " + colMeta);
                            _logger.fine("JELColumn: " + col);
                        }
                    }
                    // Consistent with:
                    // jelNames[n++] = colName
            ...
            _logger.info("Expression compilation error:" + ce.getMessage());
            _logger.info("Expression executed: \n" + expression);
        ...
        if (_logger.isLoggable(Level.FINE)) {
            _logger.fine("eval: result = " + Arrays.toString(result));
        }
        return result;
    }
    ...
    public static class VariableResolver extends DVMap {
        ...
        private int getColumnIndex(final String colName) {
            ...
            if (_logger.isLoggable(Level.FINE)) {
                _logger.fine("Unsupported column: [" + colName + "]");
            }
            return -1;
        }
    }
    ...
    protected static final class JELColumn {
        ...
        public String toString() {
            return "JELColumn{" + "name=" + name + ", values1D=" + values1D + ", values2D=" + values2D + '}';
        }
