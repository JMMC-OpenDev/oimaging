...
public final class OIWavelength extends OITable {
 ...
 /* cached analyzed data */
    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        for (float wave : getEffWave()) {
            if ((wave < 1.00E-6f || wave > 20.00E-6f) || OIFitsChecker.isInspectRules()) {
                // rule [OI_WAVELENGTH_EFFWAVE] check the EFF_WAVE column has on range [0.1.10-6 - 20.10-6]
                checker.ruleFailed(Rule.OI_WAVELENGTH_EFF_WAVE, this, OIFitsConstants.COLUMN_EFF_WAVE);
                checker.severe("EFF_WAVE are out of range");
            }
        }
