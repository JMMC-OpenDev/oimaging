...
public final class XmlOutputVisitor implements ModelVisitor {
    ...
    public void visit(final OIFitsFile oiFitsFile) {
        ...
        if (checker != null) {
            ...
            checker.appendReportAsXml(this.buffer);
        }
    ...
    private void enterOIFitsFile(final OIFitsFile oiFitsFile) {
        ...
        if (oiFitsFile != null && oiFitsFile.getAbsoluteFilePath() != null) {
            ...
            if (oiFitsFile.getImageHdu() != null) {
                final List<FitsHeaderCard> headerCards = oiFitsFile.getImageHdu().getHeaderCards();
                if (!headerCards.isEmpty()) {
                    ...
                    for (FitsHeaderCard headerCard : headerCards) {
                        ...
                        if (FitsConstants.KEYWORD_HISTORY.equals(key) || !FitsUtils.isStandardKeyword(key)) {
    ...
    private StringBuilder targetMetadata(final OIFitsFile oiFitsFile, final int index) {
        ...
        for (final String insName : oiFitsFile.getAcceptedInsNames()) {
            ...
            if (oiWavelength != null) {
                float minWavelength = oiWavelength.getEffWaveMin();
                float maxWavelength = oiWavelength.getEffWaveMax();
                int nbChannels = oiWavelength.getNWave();

                // Resolution = lambda / delta_lambda
                float resPower = oiWavelength.getResolution();
                ...
                for (Map.Entry<Double, HashSet<OIData>> entry : oiDataPerNightId.entrySet()) {
                    ...
                    for (OIData oiData : oiDataTables) {
                        ...
                        for (short id : targetIds) {
                            ...
                            if ((id == targetId) && (nightIds[idx] == nightId)) {
