...
public final class OIInspol extends OIAbstractData {
    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        checker.ruleFailed(Rule.OI_INSPOL_MJD_RANGE, this, OIFitsConstants.COLUMN_MJD_OBS);
        checker.ruleFailed(Rule.OI_INSPOL_MJD_RANGE, this, OIFitsConstants.COLUMN_MJD_END);
        checkMJDInspol(checker, getMJDObs(), getMJDEnd());
    }

    private void checkMJDInspol(OIFitsChecker checker, double[] mjdObs, double[] mjdEnd) {
        // todo add: "at row ..." + show invalid values MJD_OBS & MJD_END
        for (int i = 0; i < mjdObs.length; i++) {
            if (mjdObs[i] < 0) {
                checker.severe("invalid MJD_OBS, cannot be < 0");
            } else if (mjdObs[i] >= mjdEnd[i]) {
                checker.severe("invalid MJD_OBS, cannot be > MJD_END");
            }
        }
    }
}
