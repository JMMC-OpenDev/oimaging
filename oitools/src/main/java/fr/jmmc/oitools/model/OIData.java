...
public abstract class OIData extends OIAbstractData {
                ...
                if (oiTarget != null) {
                    ...
                    if (MathUtils.carthesianNorm(arrayXYZ[0], arrayXYZ[1], arrayXYZ[2]) > OIArray.MIN_EARTH_RADIUS) {
                        ...
                        for (int i = 0; i < nRows; i++) {
                            ...
                            // http://www.mathworks.com/matlabcentral/fileexchange/28232-toShort-julian-date-to-greenwich-apparent-sidereal-time/content/JD2GAST.m
    ...
    protected static void checkStaIndexes(final OIFitsChecker checker, final short[][] staIndexes) {
    ...
    protected static void checkCorrIndex(final OIFitsChecker checker, final OICorr oiCorr,
        ...
        for (int row = 0; row < corrindx.length; row++) {
            ...
            // rule [CORR_INDEX_1] check I >= 1
            if (idxI < 1) {
                checker.severe(colName + " index [" + idxI + "] cannot be < 1 at row " + row);
            }
            // rule [CORR_INDEX_2] check I <= ndata
            if (idxI > ndata) {
            ...
            for (int l = 0; l < nWaves; l++) {
                ...
                if (corrChecker.contains(index)) {
                    checker.severe(colName + " index [" + index + "] at [row=" + row + ", wlen=" + l + "] already used by "
                            + corrChecker.getOriginAsString(index));
                } else {
