...
public abstract class OIData extends OIAbstractData {
        ...
        for (int i = 0; i < staIndexes.length; i++) {
            ...
            for (int k = 0; k < indexes.length; k++) {
                ...
                for (int j = k + 1; j < indexes.length; j++) {
                    ...
                    checker.ruleFailed(Rule.GENERIC_STA_INDEX_UNIQ, oiData, OIFitsConstants.COLUMN_STA_INDEX);
                    if (refId == indexes[j]) {
                        checker.severe("STA_INDEX[" + refId + "] duplicated at row " + i + " at indexes " + k + " | " + j);
                    }
                }
            }
    ...
    protected static void checkCorrIndex(final OIFitsChecker checker, final OICorr oiCorr,
        ...
        if (OIFitsChecker.isInspectRules()) {
            checker.ruleFailed(Rule.GENERIC_CORRINDX_MIN, oidata, colName);
            checker.ruleFailed(Rule.GENERIC_CORRINDX_MAX, oidata, colName);
            checker.ruleFailed(Rule.GENERIC_CORRINDX_UNIQ, oidata, colName);
        } else {
            ...
            for (int row = 0; row < corrindx.length; row++) {
                ...
                if (idxI < 1) {
                    checker.ruleFailed(Rule.GENERIC_CORRINDX_MIN, oidata, colName);
                    checker.severe(colName + " index [" + idxI + "] cannot be < 1 at row " + row);
                }
                // rule [GENERIC_CORRINDX_MAX] check if the CORRINDX values <= NDATA
                if (idxI > ndata) {
                    checker.ruleFailed(Rule.GENERIC_CORRINDX_MAX, oidata, colName);
                    checker.severe(colName + " index [" + idxI + "] cannot be > NDATA [" + ndata + "] at row " + row);
                }
                ...
                for (int l = 0; l < nWaves; l++) {
                    ...
                    if (corrChecker.contains(index)) {
                        checker.ruleFailed(Rule.GENERIC_CORRINDX_UNIQ, oidata, colName);
                        checker.severe(colName + " index [" + index + "] at [row=" + row
                                + ", wlen=" + l + "] already used by "
                                + ((OIFitsChecker.isInspectRules()) ? "undifined" : corrChecker.getOriginAsString(index)));
                    } else {
     ...
     */
    public static boolean isErrorValid(final OIFitsChecker checker, final double err, OIData oidata, String colName) {// rule [GENERIC_COL_ERR] check if the UNFLAGGED *ERR column values are valid (positive or NULL)
        if (OIFitsChecker.isInspectRules()) {
            checker.ruleFailed(Rule.GENERIC_COL_ERR, oidata, colName);
        }
