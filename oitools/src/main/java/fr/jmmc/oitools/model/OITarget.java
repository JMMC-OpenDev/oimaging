    ...
    public void checkSyntax(final OIFitsChecker checker) {
        ...
        for (int i = 0; i < len; i++) {
            if (OIFitsChecker.isInspectRules()) {
                checker.ruleFailed(Rule.OI_TARGET_COORD_EXIST, this, OIFitsConstants.COLUMN_DECEP0);
                checker.ruleFailed(Rule.OI_TARGET_COORD_EXIST, this, OIFitsConstants.COLUMN_RAEP0);
                checker.ruleFailed(Rule.OI_TARGET_COORD, this, OIFitsConstants.COLUMN_DECEP0);
                checker.ruleFailed(Rule.OI_TARGET_COORD, this, OIFitsConstants.COLUMN_RAEP0);
            }
            if (Double.isNaN(coorRa[i]) || Double.isNaN(coorDec[i])) {
                // rule [OI_TARGET_COORD_EXIST] check if the TARGET RA or DEC value is not undefined
                checker.severe("TARGET coordinates : the right ascension or declination is undefined at row " + i);
            } else if (NumberUtils.equals(coorRa[i], 0.0, COORD_ZERO_EPSILON)
                    && NumberUtils.equals(coorDec[i], 0.0, COORD_ZERO_EPSILON)) {
                ...
                checker.warning("TARGET coordinates: the right ascension and declination are 0.0 at row " + i);
            }
            ...
            if (targetNames[i] == null || OIFitsChecker.isInspectRules()) {
                ...
                checker.ruleFailed(Rule.OI_TARGET_TARGET, this, OIFitsConstants.COLUMN_TARGET);
                checker.severe("TARGET undefined at row " + i);
            }
            if (targetNames[i] != null || OIFitsChecker.isInspectRules()) {
                ...
                if (refId < 1 || OIFitsChecker.isInspectRules()) {
                    ...
                    checker.ruleFailed(Rule.OI_TARGET_TARGETID_MIN, this, OIFitsConstants.COLUMN_TARGET);
                    checker.warning("TARGET_ID[" + refId + "] cannot be < 1 at row " + i);
                }
                final String refName = targetNames[i];

                if (OIFitsChecker.isInspectRules()) {
                    checker.ruleFailed(Rule.OI_TARGET_TARGETID_UNIQ, this, OIFitsConstants.COLUMN_TARGET_ID);
                    checker.ruleFailed(Rule.OI_TARGET_TARGET_UNIQ, this, OIFitsConstants.COLUMN_TARGET);
                }
                ...
                for (int j = i + 1; j < len; j++) {
                    if (refId == targetIds[j] || OIFitsChecker.isInspectRules()) {
                        ...
                        checker.severe("TARGET_ID[" + refId + "] duplicated at rows " + i + " | " + j);
                    }
                    if (refName.equals(targetNames[j]) || OIFitsChecker.isInspectRules()) {
                        ...
                        checker.severe("TARGET[" + refName + "] duplicated at rows " + i + " | " + j);
                    }
                }
            }
