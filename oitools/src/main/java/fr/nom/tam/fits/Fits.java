...
public final class Fits {
    ...
    public static long setChecksum(BasicHDU hdu)
         ...
         */
        // LAURENT: DO NOT use date in checksum to be able to validate it ...
//    final String doneAt = "as of " + FitsDate.getFitsDateString();
        final String comment = "encoded checksum";
        hdr.addValue("CHECKSUM", "0000000000000000", comment);
        ...
        /*
        // LAURENT: prepare buffer capacity
        ...
        */
        ...
        // LBO: New checksum computation using a checksum stream (only 32K buffer allocated instead of the all byte[] !)
        final ChecksumOutputStream cs = new ChecksumOutputStream();
        hdu.write(new BufferedDataOutputStream(cs));
        final long csu = cs.getChecksum();
         ...
         */
        hdr.addValue("CHECKSUM", checksumEnc(csu, true), comment);
    ...
    private static final class ChecksumOutputStream extends OutputStream {
        ...
        public void write(byte data[], int offset, int length) {
            ...
            for (int i = 0, j; i < len; i += 2) {
                 ...
                 */
                j = i << 1;
                hi += (data[j] << 8) & 0xff00 | data[j + 1] & 0xff;
                lo += (data[j + 2] << 8) & 0xff00 | data[j + 3] & 0xff;
            }

            /* The following three cases actually cannot happen since FITS records are multiples of 2880 bytes.
             */
            if (remain >= 1) {
                hi += (data[2 * len] << 8) & 0xff00;
            }
            if (remain >= 2) {
                hi += data[2 * len + 1] & 0xff;
            }
            if (remain >= 3) {
                lo += (data[2 * len + 2] << 8) & 0xff00;
            }
