...
public final class BinaryTable extends Data implements TableData {
  ...
  public BinaryTable(Header myHeader) throws FitsException {
    ...
    for (int col = 0; col < nCol; col++) {
      int len = processCol(myHeader, col);
      rowLen += len;
    }
   ...
   */
  private int processCol(final Header header, final int col) throws FitsException {

    // LAURENT : use getTrimmedStringValue instead of getStringValue :
    final String tform = header.getTrimmedStringValue("TFORM" + (col + 1));
    if (tform == null) {
      throw new FitsException("Attempt to process column " + (col + 1) + " but no TFORMn found.");
    }

    // LAURENT : use getTrimmedStringValue instead of getStringValue :
    final String tdims = header.getTrimmedStringValue("TDIM" + (col + 1));
...
// BUG FIX : LAURENT :
// complex data has two number value (float or double)
// so sizes are twice the base class size
...
// BUG FIX : LAURENT : END
   ...
   *
   * // LAURENT : method made protected to be accessible from BinaryTableHDU + javadoc
   *
   * @param tform trimmed value of the TFORM field
   * @return char representing the FITS type
   */
  protected final char getTFORMType(final String tform) {

    for (int i = 0, len = tform.length(); i < len; i++) {
   ...
   * @return char representing the FITS type
   */
  protected final char getTFORMVarType(final String tform) {
   ...
   */
  protected final int getTFORMLength(final String tform) {
   ...
   */
  private final int initialNumber(final String tform) {
  ...
  public void fillHeader(Header h) throws FitsException {
    ...
    try {
      ...
      h.setBitpix(8, "8-bit bytes");
      h.setNaxes(2, "2-dimensional binary table");
      h.setNaxis(1, rowLen, "width of row in bytes");
      h.setNaxis(2, nRow, "number of rows in table");
      h.addValue("PCOUNT", heap.size(), "size of special data area");
      h.addValue("GCOUNT", 1, "one data group (required keyword)");
      Cursor iter = h.iterator();
      iter.setKey("GCOUNT");
      iter.next();
      iter.add("TFIELDS", new HeaderCard("TFIELDS", modelRow.length, "number of fields in each row"));
  ...
  void fillForColumn(Header h, int col, Cursor iter) throws FitsException {
    ...
    iter.add(key, new HeaderCard(key, tform, "data format of field " + (col + 1)));
...
//        if (dimens[col].length > 0 && !isVarCol(col)) {
      ...
      char comma = '(';
      for (int i = dimens[col].length - 1; i >= 0; i--) {
        tdim.append(comma);
      ...
      tdim.append(')');
      key = "TDIM" + (col + 1);
      iter.add(key, new HeaderCard(key, tdim.toString(), null));
    }
  }
  ...
  private static Object[] convertToColumns(Object[][] data) {
    ...
    for (int col = 0; col < row.length; col++) {
      ...
      if (row[col] instanceof String) {
        ...
        final Class<?> base = ArrayFuncs.getBaseClass(row[col]);
        final int[] dims = ArrayFuncs.getDimensions(row[col]);
  ...
  public void updateAfterDelete(int oldNcol, Header hdr) throws FitsException {
    hdr.addValue("NAXIS1", rowLen, "width of row in bytes");
  }
}
