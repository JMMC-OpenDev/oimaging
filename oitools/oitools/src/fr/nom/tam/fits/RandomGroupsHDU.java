...
public final class RandomGroupsHDU extends BasicHDU {
   ...
   * @return <CODE>true</CODE> if this is a random groups header.
   */
  ...
  public static Data manufactureData(Header hdr) throws FitsException {
    ...
    for (int i = 0; i < gcount; i++) {
      ((Object[][]) dataArray)[i][0] =
              ((Object[]) fr.nom.tam.util.ArrayFuncs.deepClone(sampleRow))[0];
      ((Object[][]) dataArray)[i][1] =
              ((Object[]) fr.nom.tam.util.ArrayFuncs.deepClone(sampleRow))[1];
    }
  ...
  public void info() {
    ...
    if (data == null || data.length < 1 || data[0].length != 2) {
      ...
      System.out.println("    Parameters: " + fr.nom.tam.util.ArrayFuncs.arrayDescription(data[0][0]));
      System.out.println("    Data:" + fr.nom.tam.util.ArrayFuncs.arrayDescription(data[0][1]));
    }
  }
}
