...
public final class RandomGroupsData extends Data {
  ...
  protected void fillHeader(Header h) throws FitsException {
    ...
    Class<?> pbase = fr.nom.tam.util.ArrayFuncs.getBaseClass(paraSamp);
    Class<?> dbase = fr.nom.tam.util.ArrayFuncs.getBaseClass(dataSamp);

    if (pbase != dbase) {
      throw new FitsException("Data and parameters do not agree in type for random group");
    }

    final int[] pdims = fr.nom.tam.util.ArrayFuncs.getDimensions(paraSamp);
    final int[] ddims = fr.nom.tam.util.ArrayFuncs.getDimensions(dataSamp);
