...
public abstract class BasicHDU implements FitsElement {
   ...
   * 		blanks stripped.
   */
  public final String getTrimmedString(final String keyword) {
  ...
  void setPrimaryHDU(boolean newPrimary) throws FitsException {
    ...
    if (isPrimary) {
      ...
      if (card == null) {
        ...
        myHeader.nextCard();
        myHeader.addValue("EXTEND", true, "FITS dataset may contain extensions");
      }
    }
  ...
  /**
   * LAURENT : fixed ImageData bug where NAXIS = 0 and NAXIS1 exists !
   * @return empty Image HDU
   */
  public static BasicHDU getDummyHDU() {
    try {
      final ImageData data = new ImageData((Object) null);
      final Header header = ImageHDU.manufactureHeader(data);
